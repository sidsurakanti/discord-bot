...
class Coronavirus(commands.Cog, name="Coronavirus"):
	...
	async def cases(self, ctx, *, country: str):
		...
		try:
			...
			embed = discord.Embed(description=f"Country not found", 
		...
		embed = discord.Embed(color=0xC36865, timestamp=ctx.message.created_at)
		...
		embed.set_footer(text="Powered by covid19api.com")
	...
	async def world(self, ctx):
		...
		# creating and sending embed
		embed = discord.Embed(timestamp=ctx.message.created_at, color=0xC36865)
...
async def setup(bot):
	await bot.add_cog(Coronavirus(bot=bot))
